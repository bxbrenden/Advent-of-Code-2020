 ...
 *       Filename:  day01.c
 *
 *    Description: Day 01 of Advent of Code 2020, part 2 
 *
 *        Version:  1.0
...
int solve_puzzle(int *vals, int num_newlines) {
	int outer, inner, thinner;

	for (outer=0; outer<num_newlines; outer++) {
		for (inner=0; inner<num_newlines; inner++) {
			for (thinner=0; thinner<num_newlines; thinner++) {
				if (vals[outer] + vals[inner] + vals[thinner] == 2020) {
					printf("%d + %d + %d == 2020\n", vals[outer], vals[inner], vals[thinner]);
					return vals[outer] * vals[inner] * vals[thinner];
				}
			}
		}
	}
